...
class AttentionLstmHead(BaseHead):
    ...
    def __init__(self,
        ...
        self.fc_out1 = paddle.nn.Linear(in_features=self.lstm_size * 2* self.feature_num,
                                        out_features=2048, #4096,
                                        bias_attr=ParamAttr(
                                            regularizer=L2Decay(0.0),
                                            initializer=Normal()))
        self.relu = paddle.nn.ReLU()
        # self.fc_out2 = paddle.nn.Linear(in_features=4096,
        #                                 out_features=2048,
        #                                 bias_attr=ParamAttr(
        #                                     regularizer=L2Decay(0.0),
        #                                     initializer=Normal()))
        self.fc_logit = paddle.nn.Linear(in_features=2048,
    ...
    def forward(self, inputs):
        ...
        fc_logit = self.fc_logit(fc_out1_act)
    ...
    def loss(self, lstm_logit, labels, **kwargs):
        ...
        sum_cost = F.cross_entropy(lstm_logit, labels) #bce_logit_loss(lstm_logit, labels)
        return sum_cost

    def metric(self, lstm_output, labels):
        pred = lstm_output.numpy()
        label = labels.numpy()
        top1 = paddle.metric.accuracy(input=lstm_output, label=labels, k=1)
        top5 = paddle.metric.accuracy(input=lstm_output, label=labels, k=5)
        return top1,top5
